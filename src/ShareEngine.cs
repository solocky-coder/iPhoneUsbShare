...
public sealed class ShareEngine
{
    ...
    public void WriteLog(string message)
    {
        ...
        try
        {
            lock (LogFileLock)
            {
                File.AppendAllText(ActivityLogPath, line + Environment.NewLine, new UTF8Encoding(false));
            }
        }
        catch { }
    ...
    public async Task EnsurePrerequisitesAsync()
    {
        ...
        if (!File.Exists(sysDst))
        {
            File.Copy(sys, sysDst, false);
            WriteLog($"Copied libusb0.sys to {sysDst}");
        }
        ...
        try
        {
            ...
            if (svc.Status != System.ServiceProcess.ServiceControllerStatus.Running)
            {
                ...
                WriteLog("libusb0 service started.");
            }
        }
    ...
    public Task StopAsync()
    {
        try
        {
            ...
            WriteLog("Sharing stopped and Apple USB configuration restored.");
        }
    ...
    {
        WriteLog($"Configuring Internet Connection Sharing: {wifi} -> {phoneAdapter}");
        DisableAllIcs();
        ...
        WriteLog("Internet Connection Sharing enabled.");
    }
    ...
    private static string? FindLease(string adapterName)
    {
        try
        {
            ...
            foreach (var ua in nic.GetIPProperties().UnicastAddresses)
                if (ua.Address.AddressFamily == System.Net.Sockets.AddressFamily.InterNetwork && ua.Address.ToString().StartsWith(Subnet, StringComparison.Ordinal)) return ua.Address.ToString();
        }
        catch { }
        return null;
    }

    private static NetworkInterface? FindWifi() => NetworkInterface.GetAllNetworkInterfaces()
        .Where(n => n.OperationalStatus == OperationalStatus.Up)
        .Where(n => n.NetworkInterfaceType is NetworkInterfaceType.Wireless80211)
        .OrderByDescending(n => n.Speed)
        .FirstOrDefault();

    private static NetworkInterface? FindPhoneAdapter() => NetworkInterface.GetAllNetworkInterfaces()
        .FirstOrDefault(n => n.Name.Contains("Ethernet", StringComparison.OrdinalIgnoreCase) && n.Description.Contains("Apple", StringComparison.OrdinalIgnoreCase));

    private static (double rx, double tx) GetRates(string name)
    {
        try
        {
            var n = NetworkInterface.GetAllNetworkInterfaces().FirstOrDefault(x => x.Name == name);
            return n is null ? (0, 0) : (n.GetIPv4Statistics().BytesReceived, n.GetIPv4Statistics().BytesSent);
        }
        catch { return (0, 0); }
    }
    ...
    private readonly record struct PnpDevice(string Id, string Name);
}
